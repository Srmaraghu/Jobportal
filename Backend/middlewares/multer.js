...
const profilePicFilter = (req, file, cb) => {
  if (file.mimetype === 'image/jpeg' || file.mimetype === 'image/png') {
    ...
    cb(new Error("Only JPEG and PNG files are allowed for profile picture"), false);
  }
};

// Define file filter for both `profilePic` and `resume` in `updateProfile`
const updateProfileFilter = (req, file, cb) => {
  if (file.fieldname === 'profilePic' && (file.mimetype === 'image/jpeg' || file.mimetype === 'image/png')) {
    cb(null, true);
...
const singleUpload = multer({ storage, limits, fileFilter: profilePicFilter }).single('profilePic');

// Multiple file upload (profilePic and resume) for profile update
const multiUpload = multer({ storage, limits, fileFilter: updateProfileFilter }).fields([
